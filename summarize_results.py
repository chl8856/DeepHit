...
from utils_eval import weighted_brier_score, weighted_c_index
...
for out_itr in range(OUT_ITERATION):
    ...
    initial_W = tf.compat.v1.keras.initializers.VarianceScaling(
    ...
    saver.restore(
        sess, in_path + "/itr_" + str(out_itr) + "/models/model_itr_" + str(out_itr)
    )
    ...
    print(
        "SharedNet Parameters: "
        + "h_dim_shared = "
        + str(h_dim_shared)
        + " num_layers_shared = "
        + str(num_layers_shared)
        + "Non-Linearity: "
        + str(active_fn)
    )
    print(
