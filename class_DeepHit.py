...
class Model_DeepHit:
    ...
    def _build_net(self):
        with tf.compat.v1.variable_scope(self.name):
            ...
            self.lr_rate = tf.compat.v1.placeholder(
            ...
            self.x = tf.compat.v1.placeholder(
                tf.float32, shape=[None, self.x_dim], name="inputs"
            )
            self.k = tf.compat.v1.placeholder(
                tf.float32, shape=[None, 1], name="labels"
            )  # event/censoring label (censoring:0)
            self.t = tf.compat.v1.placeholder(
                tf.float32, shape=[None, 1], name="timetoevents"
            )

            self.fc_mask1 = tf.compat.v1.placeholder(
                tf.float32,
                shape=[None, self.num_Event, self.num_Category],
                name="mask1",
            )  # for Loss 1
            self.fc_mask2 = tf.compat.v1.placeholder(
                tf.float32, shape=[None, self.num_Category], name="mask2"
            )  # for Loss 2 / Loss 3

            # SHARED SUBNETWORK w/ FCNETS
            shared_out = utils.create_FCNet(
                self.x,
            ...
            self.solver = tf.compat.v1.train.AdamOptimizer(
    ...
    def loss_Log_Likelihood(self):
        ...
        tmp1 = tf.reduce_sum(
            tf.reduce_sum(self.fc_mask1 * self.out, axis=2),
            axis=1,
            keepdims=True,
        )
        tmp1 = I_1 * log(tmp1)
        ...
        tmp2 = tf.reduce_sum(
            tf.reduce_sum(self.fc_mask2 * self.out, axis=2),
            axis=1,
            keepdims=True,
        )
    ...
    def loss_Ranking(self):
        ...
        for e in range(self.num_Event):
            ...
            T = tf.nn.relu(
                tf.sign(
                    tf.matmul(one_vector, tf.transpose(self.t))
                    - tf.matmul(self.t, tf.transpose(one_vector))
                )
            )
        ...
        eta = tf.reduce_mean(
            tf.reshape(eta, [-1, self.num_Event]), axis=1, keepdims=True
        )
